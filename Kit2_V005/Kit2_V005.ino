...
const char* thingsboardURL = "https://149.56.18.149:32782/api/v1/e8X11ekQjvdHWQAIyfDT/telemetry";
const char* thingsboardRelayURL = "https://149.56.18.149:32782/api/v1/e8X11ekQjvdHWQAIyfDT/attributes?sharedKeys=relay";
...
void loop() {
  ...
  Serial.println(relayStatus ? "Relay Status: ON" : "Relay Status: OFF");
  ...
  tft.drawGFXText(valueX, y, isnan(pf) ? "---" : String(pf, 2), COLOR_WHITE); y += spacing;

  tft.drawGFXText(labelX, y, "Relay:", COLOR_CYAN);
  tft.drawGFXText(valueX, y, relayStatus ? "ON" : "OFF", COLOR_WHITE);
  ...
  if (https.begin(client, thingsboardURL)) {
    ...
    doc["pf"] = pf;
    doc["relay"] = relayStatus;  // include relay status
    String json;
  ...
  // Update Relay State from ThingsBoard
  controlRelay();
...
void controlRelay() {
  ...
  if (https.begin(client, thingsboardRelayURL)) {
    int httpCode = https.GET();

    if (httpCode == HTTP_CODE_OK) {
      String payload = https.getString();
      Serial.println("ThingsBoard Relay Response: " + payload);
      ...
      Serial.printf("Relay Check HTTP Error: %d\n", httpCode);
    }
    https.end();
  } else {
    Serial.println("ThingsBoard Relay Connection failed.");
  }
}
