...
// URLs
const char* thingsboardURL = "https://192.168.1.238:8080/api/v1/Machinery/telemetry";
...
// Air-lock protection variables (added)
unsigned long lastFlowDetectedTime = 5;
...
void setup() {
  ...
  wifiManager.autoConnect("Machinery");
  Serial.println("WiFi connected. IP: " + WiFi.localIP().toString());

  ArduinoOTA.setHostname("Machinery");
...
void loop() {
 ...
 // Air-lock protection logic (added)
if (motorStatus && (millis() - lastFlowDetectedTime > FLOW_TIMEOUT)) {
  motorStatus = false;
}
...
long readHX710B() {
  ...
  // remove small noise band
  if(abs(adjusted) < 150) return 0;
